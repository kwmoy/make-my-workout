...
def get_all_muscles_worked_on_date(date: str) -> list[str]:
    ...
    return sum(
        [get_muscles_worked(exercise) for exercise in workout_log[date]], []
    )
...
# muscles_worked_list = sum(
# [get_all_muscles_worked_on_date(str_date)
# for str_date in ["2024-01-10","2024-01-11"]],
# []
# )
muscles_worked_list = sum(
    [
        get_all_muscles_worked_on_date(str_date)
        for str_date in workout_log.keys()
    ],
    [],
)
muscles_dict = {
    muscle: muscles_worked_list.count(muscle)
    for muscle in set(muscles_worked_list)
}
print(muscles_dict)
